...
- **Preserve the original cause** — when translating an external or infrastructure exception, raise the typed boundary exception with `raise ... from e`. Do not replace it with a generic exception that discards the traceback or stable failure category.
